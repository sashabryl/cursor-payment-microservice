...
async def create_order_endpoint(api_key: str, order_data: OrderCreate, db_session=Depends(get_db_session)):
...
async def update_order_status_endpoint(api_key: str, order_id: int, new_status: models.PaymentStatus, db_session=Depends(get_db_session)):
    check_api_key(api_key)
...
async def delete_order_endpoint(api_key: str, order_id: int, db_session=Depends(get_db_session)):
    check_api_key(api_key)
